...
class Layer:
  ...
  def forward(self, Aprev):
    ...
    elif self.type == 'Softmax': self.A = self.softmax(self.Z)
  ...
  def backward(self, dA, batch_size, dZ=None):
    ...
    elif self.type == 'Softmax': g_primeZ = self.softmax(self.Z, True)
    ...
    self.dAprev = np.dot(self.W.T, self.dZ)  #  self.W.T
...
### class L Layer NN
class LLayerNN:
  def __init__(self, featureVectorSize=1, layers=None, num_classes=1):    # 'layers' specifies just hidden ones
    ...
    self.outputLayer = Layer('Sigmoid', num_classes, self.hidden_layers[-1].size)
    self.norms = None
    pass
  ...
  def train(self, X, Y, minibatchSize=128, alfa = 0.01, epochsNum = 100000):
    ...
    for j in range(epochsNum):
      ...
      if j % int(0.1*epochsNum) == 0:
        print("Error after epoch {} (from {}): {}".format(j, epochsNum, costv))
