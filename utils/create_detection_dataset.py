...
def pick_up_read_one(images_dict):
...
def pick_up_read_set(labels_images_dict, amount):
    labels = list(labels_images_dict.keys())
    res_set = []
    while amount > 0:
        label = labels[randint(0, len(labels)-1)]
        image = pick_up_read_one(labels_images_dict[label])
...
def main(args):
    ...
    while amount > 0:
        foreground_images = pick_up_read_set(foregrounds, amount_per_image)
